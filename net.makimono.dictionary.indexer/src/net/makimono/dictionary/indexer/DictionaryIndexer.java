...
public class DictionaryIndexer extends AbstractJaxbIndexer<JMdict, Entry> {
	...
	protected Document createDocument(Entry jmdictEntry) throws Exception {
		...
		for (net.makimono.dictionary.model.Sense sense : entry.getSenses()) {
	...
	private byte[] getSerializedEntry(net.makimono.dictionary.model.DictionaryEntry entry) throws IOException {
		...
		net.makimono.dictionary.model.DictionaryEntry.writeEntry(objectOutputStream, entry);
	...
	DictionaryEntry transformEntry(Entry jmdictEntry) throws Exception {
		DictionaryEntry entry = new net.makimono.dictionary.model.DictionaryEntry();
		...
		for (Sense jmdictSense : jmdictEntry.getSense()) {
			net.makimono.dictionary.model.Sense sense = new net.makimono.dictionary.model.Sense();
			...
			for (Gloss gloss : jmdictSense.getGloss()) {
				net.makimono.dictionary.model.Meaning meaning = new net.makimono.dictionary.model.Meaning();
	...
	private DictionaryEntry mixinAdditionalMeanings(DictionaryEntry entry) {
		...
		for (String key : keys) {
			if (mixinMeanings.containsKey(key)) {
				net.makimono.dictionary.model.Sense mixinSense = mixinMeanings.remove(key).getSenses().get(0);

				for (net.makimono.dictionary.model.Sense s : entry.getSenses()) {
